...
class VadOptions(NamedTuple):
    ...
    # threshold: float = 0.5  # gw: 0.3 # rep 0.5
    min_speech_duration_ms: int = 30  # original & gw: 250
    max_speech_duration_s: float = float("inf")
...
class VADModelSingleton:
    """
    VAD模型单例类
    确保整个应用程序中只有一个VAD模型实例
    """
    _instance = None
    _model = None
    _initialized = False
    ...
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(VADModelSingleton, cls).__new__(cls)
    ...
    def get_initial_state(self, batch_size: int):
        ...
        return self._model.get_initial_state(batch_size)
    
    def __call__(self, x, state, sr: int):
        """调用模型进行推理"""
        if self._model is None:
            raise RuntimeError("VAD模型未加载")
        return self._model(x, state, sr)


# 全局单例实例
_vad_singleton = VADModelSingleton()


def get_vad_model():
    """Returns the VAD model singleton instance."""
    return _vad_singleton.get_model()


def get_vad_singleton():
    """获取VAD模型单例实例"""
...
def get_speech_timestamps(
    ...
    **kwargs,
    ...
    sampling_rate = 16000
    min_speech_samples = sampling_rate * \
        min_speech_duration_ms / 1000  # 如果间隔区间没这个长度就不会添加
    ...
    min_silence_samples = sampling_rate * min_silence_duration_ms / \
        1000  # 在每个silent需要等 min_silence_duration_ms 后才结束，
    min_silence_samples_at_max_speech = sampling_rate * \
        98 / 1000  # 0.098s # need to adjust？
...
#         # 确保 vad_options 被正确初始化
#         if vad_options is None:
#             vad_options = VadOptions()  # 使用默认参数
...
#         # print('vad_audio.shape: {}'.format(vad_audio.shape))
