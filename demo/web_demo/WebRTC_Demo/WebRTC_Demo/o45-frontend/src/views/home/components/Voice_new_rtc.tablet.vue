<!--
    ...
    文件路径：src/views/home/components/Voice_new_rtc.tablet.vue
-->
<template>
    ...
    <!-- 可拖动的字幕悬浮窗（全局定位，可在整个页面拖动） -->
    <div
        v-if="showText"
        class="draggable-subtitle-panel"
    ...
    >
        <div class="subtitle-header">
            ...
            <span class="drag-handle">{{ t('captions') }}</span>
    ...
    <div class="tablet-layout-container" ref="tabletLayoutContainer">
        <!-- 主语音页面 -->
            ...
            <VoiceSelectorDialog ref="selectorDialog" @confirm="handleVoice" /> -->
                <div class="footer-btn">
                    ...
                    <section v-if="isCalling && state.status && state.status !== 'connecting'">
                        <div class="text-btn">
                            <SvgIcon name="text" class="text-icon" @click="showText = !showText" />
                        </div>
...
<script setup>
    ...
    // 字幕面板拖动相关
    const subtitlePosition = ref({ x: 0, y: 0 }); // 初始位置将在 mounted 时根据容器位置计算
    const isDragging = ref(false);
    const dragOffset = ref({ x: 0, y: 0 });

    const subtitlePanelStyle = computed(() => ({
        left: `${subtitlePosition.value.x}px`,
        top: `${subtitlePosition.value.y}px`
    }));
    ...
    // 设置字幕初始位置（基于容器位置）
    const setInitialSubtitlePosition = () => {
        if (tabletLayoutContainer.value) {
            const rect = tabletLayoutContainer.value.getBoundingClientRect();
    ...
    // 拖动开始
    const startDrag = e => {
        // 如果点击的是滚动内容区域或关闭按钮，不触发拖动
        if (e.target.closest('.subtitle-content') || e.target.closest('.close-btn-wrapper')) {
            return;
        }
        ...
        // 计算鼠标/触摸点相对于字幕面板的偏移
        dragOffset.value = {
            x: clientX - subtitlePosition.value.x,
            y: clientY - subtitlePosition.value.y
        };
    ...
    const onDrag = e => {
        ...
        newX = Math.max(-220, Math.min(newX, maxX)); // 允许部分拖出视口
        newY = Math.max(-300, Math.min(newY, maxY));

        subtitlePosition.value = { x: newX, y: newY };
    };
