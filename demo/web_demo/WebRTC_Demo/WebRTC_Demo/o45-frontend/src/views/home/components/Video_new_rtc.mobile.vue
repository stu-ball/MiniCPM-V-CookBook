<!--
    ==================== 手机端视频通话组件 ====================
    独立的手机端代码，适配全屏布局
    文件路径：src/views/home/components/Video_new_rtc.mobile.vue
-->
<template>
    ...
    <div class="mobile-layout-container">
        <!-- <div class="video-clarity-row" v-if="!isCalling">
            <div class="video-clarity-toggle" :class="{ disabled: isCalling }">
        ...
        <div class="high-refresh-badge" v-if="isCalling && isHighRefresh">高刷</div>
        <!-- 主视频页面 -->
        <div class="video-page" v-loading="loading" element-loading-background="transparent">
            <div class="video-page-container">
                ...
                >
                    ...
                    isCalling: {{ isCalling }}<br />
                    loading: {{ loading }}
                </div>
            </div>
            <!-- 开始通话按钮（未通话时，需要避开底部切换栏）-->
            <div class="video-page-btn-start" v-if="!isCalling && !callLoading">
            ...
            <!-- callLoading状态：只显示动画 -->
            <div class="video-page-btn-loading" v-else-if="callLoading">
                <VoiceGifCopy
                    :status="state.status"
            ...
            <!-- 通话中状态：按钮组（底部切换栏已隐藏，距离底部16px）-->
            <div class="video-page-btn-calling" v-else>
                <!-- 第一行：动画 -->
                ...
                <div class="calling-buttons">
                    ...
                    >
                        <div class="btn-circle">
                            <SvgIcon name="mobile-interrupt" class="btn-icon" />
                        </div>
                    ...
                    <div class="btn-item" @click="toggleMic">
                        <div class="btn-circle" :class="{ 'mic-off': !state.audioEnabled }">
                            <SvgIcon v-if="state.audioEnabled" name="mobile-microphone-on" class="btn-icon" />
                    ...
                    <div class="btn-item" @click="stopRecording">
                        <div class="btn-circle btn-circle-end">
                            <SvgIcon name="hang-up" class="btn-icon btn-icon-end" />
                        </div>
...
<script setup>
    ...
    //         switchCamera
    //     };
    //     console.log('💡 调试工具已就绪，可在控制台使用:');
    //     console.log('  - await window.$livekit.checkCurrentCamera() // 检查当前摄像头');
    //     console.log('  - window.$livekit.clearCameraCache()         // 清除缓存重新检测');
    //     console.log('  - window.$livekit.compareFieldOfView()       // 视野对比测试');
    // }
    ...
    const initRecording = async () => {
        ...
        try {
            ...
            console.error('initRecording failed:', e);
            ElMessage({
                type: 'error',
                message: e?.message || t('callErrMsg'),
                duration: 3000,
                customClass: 'system-error'
            });
        } finally {
...
<style lang="less" scoped>
    ...
    .high-refresh-badge {
        ...
        border-radius: 104.794px;
        background: rgba(0, 0, 0, 0.3);
        backdrop-filter: blur(5.31562614440918px);
        color: #fff;
    ...
    .video-page {
        ...
        &-btn-start {
            position: absolute;
            bottom: 140px; /* 避开底部切换栏：62px+20px+34px+24px=140px */
            left: 0;
            right: 0;
            display: flex;
            align-items: center;
        ...
        &-btn-calling {
            position: absolute;
            bottom: 16px;
            bottom: calc(16px + env(safe-area-inset-bottom)); /* iPhone 安全区域支持 */
            left: 0;
            right: 0;
            display: flex;
            flex-direction: column;
            align-items: center;
            width: 100%;
            gap: 56px; /* VoiceGifSmall 和按钮组间距6px */
            ...
            .calling-buttons {
                ...
                .btn-item {
                    ...
                    .btn-circle {
                        ...
                        height: 70px;
                        border-radius: 50%;
                        background: rgba(0, 0, 0, 0.3); /* 视频通话使用半透明白色背景 */
                    ...
                    .btn-circle.mic-off {
                        background: rgba(0, 0, 0, 0.6); /* 视频通话使用半透明白色背景 */
                    }

                    .btn-label {
                        margin-top: 6px;
                        color: #ffffff; /* 视频通话背景较深，使用白色文字 */
                        // font-family: 'PingFang SC';
                        font-size: 12px;
                        font-style: normal;
                        font-weight: 400;
