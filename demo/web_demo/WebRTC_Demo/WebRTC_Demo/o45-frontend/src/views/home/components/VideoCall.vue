...
<script setup>
    ...
    const stopRecording = async () => {
        ...
        await (myvad && myvad.destroy());
        await audioStreamPlayer?.port?.postMessage({ type: 'stop' });
        await resetAudioWorklet();
        console.log('stop!!!!!!!');
    };
    defineExpose({
        stopRecording
    ...
    let audioPlayContext;
    let audioStreamPlayer;
    // 重置状态
    const resetAudioWorklet = () => {
        if (audioStreamPlayer) {
            audioStreamPlayer.disconnect();
            audioStreamPlayer = null;
        }
        if (audioPlayContext) {
            audioPlayContext.close();
            audioPlayContext = null;
        }
    };
    // 设置 AudioWorklet
    async function setupAudioWorklet() {
        // 每次先重置状态，再开始新的
        await resetAudioWorklet();
        audioPlayContext = new AudioContext();
        await audioPlayContext.audioWorklet.addModule('audio-stream-player.js');
        audioStreamPlayer = new AudioWorkletNode(audioPlayContext, 'audio-stream-player');
        audioStreamPlayer.connect(audioPlayContext.destination);
        audioStreamPlayer.port.onmessage = event => {
            console.log('接收到finish', isEnd.value);
            // 播放完成并且是结束状态时，重连
            if (event.data.type === 'finished' && isCalling.value) {
            ...
            async onmessage(msg) {
                ...
                if (data.choices[0]?.audio) {
                    ...
                    if (!getStopValue() && isCalling.value) {
                        ...
                        playing.value = true;
                        audioQueue.push(data.choices[0]?.audio);
                        processAudioQueue();
                        // const arrayBuffer = base64ToArrayBuffer(data.choices[0]?.audio);
                        // const audioBuffer = await audioPlayContext.decodeAudioData(arrayBuffer);
                        // audioStreamPlayer.port.postMessage({ type: 'add', audioData: audioBuffer.getChannelData(0) });
                    }
                } else {
                    // 发生异常了，直接重连
                    // buildConnect('异常重连');
                    audioStreamPlayer.port.postMessage({ type: 'finish' });
                }
    ...
    const skipVoice = async () => {
        ...
        if (!skipDisabled.value) {
            ...
            stop.value = true;
            audioDOM?.pause();
            audioStreamPlayer?.port?.postMessage({ type: 'skip' });
            setTimeout(() => {
                skipDisabled.value = true;
            }, 300);
            try {
