<!--
    ==================== 平板端视频通话组件 ====================
    复用PC端逻辑，可在此基础上针对平板设备进行UI和交互优化
    文件路径：src/views/home/components/Video_new_rtc.tablet.vue
-->
<template>
    <div class="remote-audio" v-for="(tracks, sid) in state.remoteTracks" :key="sid">
        <audio :ref="setRemoteAudioRef(sid)" autoplay></audio>
    </div>
    <div class="tablet-layout-container" :class="{ 'show-subtitle': showText, landscape: isLandscape }">
        <!-- 字幕区域（平板专用，不可拖动） -->
        <div v-if="showText" class="subtitle-panel">
        ...
        <!-- 主视频页面 -->
        <div class="video-page" v-loading="loading" element-loading-background="transparent">
            ...
            <div class="video-page-btn">
                <!-- 默认状态：开始按钮 -->
                <div class="btn-start-box" v-if="!isCalling && !callLoading">
                    <SvgIcon name="start" class="start-icon" @click="initRecording" />
                    <div class="footer-tips">{{ t('videoCallBtn') }}</div>
                </div>

                <!-- callLoading状态：只显示动画 -->
                <div class="btn-loading-box" v-else-if="callLoading">
                    <VoiceGifCopy
                        :status="state.status"
                ...
                <!-- 通话中状态：两行布局 -->
                <div class="btn-calling-box" v-else>
                    <!-- 第一行：动画 -->
                    ...
                    <div class="calling-buttons">
                        ...
                        >
                            <div class="btn-circle">
                                <SvgIcon name="tablet-interrupt" class="btn-icon" />
                            </div>
                        ...
                        <div class="btn-item" @click="toggleMic">
                            <div class="btn-circle">
                                <SvgIcon v-if="state.audioEnabled" name="microphone-on" class="btn-icon" />
                                <SvgIcon v-else name="microphone-off" class="btn-icon microphone-off" />
                            </div>
                        ...
                        <div class="btn-item" @click="stopRecording">
                            <div class="btn-circle">
                                <SvgIcon name="quit" class="btn-icon" />
                            </div>
...
<script setup>
    ...
    function startVideoElementHealthMonitoring() {
        ...
        console.log('🏥 [iPad] 启动 video 元素健康监控，每 10 秒检查一次');
    ...
    function stopVideoElementHealthMonitoring() {
        if (videoElementHealthTimer) {
            ...
            videoElementRecoveryAttempts = 0;
            console.log('🏥 [iPad] video 元素健康监控已停止');
        }
    }

    /**
     * 🔧 检查 video 元素渲染健康状态（iPad 端）
     */
    function checkVideoElementHealth() {
        try {
            ...
            if (videoWidth === 0 || videoHeight === 0) {
                console.warn('⚠️ [iPad] Video 元素未渲染内容:', { videoWidth, videoHeight, readyState });

                // 尝试恢复
                if (videoElementRecoveryAttempts < MAX_VIDEO_ELEMENT_RECOVERY) {
                    videoElementRecoveryAttempts++;
                    console.log(
                        `🔄 [iPad] 尝试恢复 video 元素渲染 (第 ${videoElementRecoveryAttempts}/${MAX_VIDEO_ELEMENT_RECOVERY} 次)`
                    );
                    recoverVideoElement();
                } else {
                    console.error('❌ [iPad] Video 元素恢复失败，已达到最大尝试次数');
                    ElMessage({
                        type: 'warning',
            ...
            if (paused && readyState >= 2) {
                console.warn('⚠️ [iPad] Video 元素已暂停，尝试恢复播放');
                video.play().catch(err => {
                    console.error('❌ [iPad] 恢复播放失败:', err);
                });
            }

            // 检查 3: readyState 是否正常
            if (readyState < 2) {
                console.warn('⚠️ [iPad] Video 元素 readyState 异常:', readyState);
            }

            // 如果一切正常，重置恢复计数器
            if (videoWidth > 0 && videoHeight > 0 && videoElementRecoveryAttempts > 0) {
                console.log('✅ [iPad] Video 元素已恢复正常，重置恢复计数器');
                videoElementRecoveryAttempts = 0;
            }
        } catch (error) {
            console.error('❌ [iPad] Video 元素健康检查出错:', error);
        }
    }

    /**
     * 🔧 恢复 video 元素渲染（iPad 端）
     */
    function recoverVideoElement() {
        try {
            console.log('🔄 [iPad] 开始恢复 video 元素...');

            const video = videoRef.value;
            if (!video) {
                console.error('❌ [iPad] Video 元素不存在');
                return;
            }
            ...
            if (!videoTrack || !videoTrack.mediaStreamTrack) {
                console.error('❌ [iPad] 未找到视频轨道');
                return;
            }

            // 方法 1: 重新 attach 视频轨道
            console.log('🔄 [iPad] 方法 1: 重新 attach 视频轨道');
            videoTrack.detach(video);
            setTimeout(() => {
                videoTrack.attach(video);
                // 确保播放
                video.play().catch(err => console.warn('[iPad] 播放失败:', err));
                console.log('✅ [iPad] 视频轨道已重新 attach');
            }, 100);
            ...
            setTimeout(() => {
                ...
                if (currentWidth === 0 || currentHeight === 0) {
                    console.log('🔄 [iPad] 方法 2: 重新设置 srcObject');
                    ...
                    if (mediaTrack && mediaTrack.readyState === 'live') {
                        ...
                        video.play().catch(err => console.warn('[iPad] 播放失败:', err));
                        console.log('✅ [iPad] srcObject 已重新设置');
                    } else {
                        console.error('❌ [iPad] MediaStreamTrack 状态异常:', mediaTrack?.readyState);
                    }
                }
            }, 1000);
            ...
            setTimeout(() => {
                ...
                if (currentWidth === 0 || currentHeight === 0) {
                    console.log('🔄 [iPad] 方法 3: 强制刷新 video 元素');
                    // 触发 DOM 重绘
                    video.style.display = 'none';
                    setTimeout(() => {
                        video.style.display = 'block';
                        video.play().catch(err => console.warn('[iPad] 播放失败:', err));
                        console.log('✅ [iPad] Video 元素已强制刷新');
                    }, 50);
                }
            }, 2000);
        } catch (error) {
            console.error('❌ [iPad] 恢复 video 元素失败:', error);
        }
    }
    ...
    // 🔧 修复：直接使用 state.status，不自己计算状态
    // 原因：自己计算的状态基于 remoteAudioActive，可能在音频还没播完时就提前切换
    ...
    onMounted(() => {
        ...
        registerLocalVideoElement(videoRef.value);
        // 检测横竖屏
        checkOrientation();
    ...
    onBeforeUnmount(() => {
        registerLocalVideoElement(null);
        // 移除横竖屏监听
        window.removeEventListener('resize', checkOrientation);
        window.removeEventListener('orientationchange', checkOrientation);

        // 🔧 清理 video 元素健康监控（iPad 端）
    ...
    // 切换后校验 video 绑定，避免多次切换后黑屏
    const ensureVideoBound = async () => {
        const el = videoRef.value;
        if (!el || mode.value !== 'video') return;
        const vt = state.localTracks.find(t => t.kind === 'video');
        if (!vt?.mediaStreamTrack) return;

        const currentTrack = el.srcObject?.getVideoTracks?.()[0];
        if (!currentTrack || currentTrack.id !== vt.mediaStreamTrack.id) {
            el.srcObject = new MediaStream([vt.mediaStreamTrack]);
        }
    ...
    const initRecording = async () => {
        ...
        const initConfig = {
            ...
            highRefresh: localStorage.getItem(highRefreshCacheKey) === 'true'
        };
...
<style lang="less" scoped>
    ...
    .tablet-layout-container {
        ...
        /* 显示字幕时，改为 visible 避免 box-shadow 被截断 */
        &.show-subtitle {
            overflow: visible;
        }
    ...
    .video-page {
        ...
        &-btn {
            ...
            .btn-loading-box {
                ...
                align-items: center;
                width: 100%;
                height: 100px; /* 与 calling-animation 保持相同高度，避免位置跳动 */
                position: relative;
            }
            ...
            .btn-calling-box {
                ...
                .calling-animation {
                    ...
                    margin-bottom: 40px;
                    width: 100%;
                    height: 100px; /* VoiceGifSmall 需要固定高度：68px 圆形 + 文字空间 */
                    position: relative;
                }
