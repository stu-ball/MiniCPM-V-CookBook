    ...
    <div class="video-page">
            ...
            <div class="header-left">
                ...
                <div class="voice-container" v-else>
                    <Voice
                        ...
                        :isCalling="isCalling"
                        :isPlaying="playing"
                        :configList="voiceConfigList"
                        :boxStyle="{ height: '34px' }"
                        :itemStyle="{ width: '2.7px', margin: '0 0.5px' }"
                    />
                </div>
            </div>
            <div class="header-right">
                <!-- <div class="text-input">
                <span class="input-label">{{ t('textInput') }}</span>
                <el-tooltip popper-class="config-tooltip" effect="dark" :content="t('textInputTips')" placement="top">
                    <SvgIcon name="question" class="question-icon" />
                </el-tooltip>
                <el-switch v-model="isTextModel" inline-prompt size="small" class="switch-input" />
            </div> -->
            </div>
        </div>
        <div class="video-page-content">
            <div class="video-page-content-video" v-loading="loading" element-loading-background="#f3f3f3">
                ...
                <div class="switch-camera-icon" v-if="isMobile() && isCalling && !loading" @click="switchCamera">
            ...
            <div class="video-page-content-right">
                ...
                <div class="input-box" v-if="isCalling && isTextModel">
                    <InputArea v-model:query="query" v-model:sendLoading="sendLoading" @sendText="handleSendText" />
                </div>
                <div class="skip-box" v-if="isCalling">
                    <!-- <DelayTips
...
<script setup>
        ...
        fetchEventSource(url, {
            ...
            onmessage(msg) {
                ...
                if (data.choices[0]?.text) {
                    ...
                    tempText = tempText.replace('<audit_end>', str);
                ...
                if (data.choices[0].text.includes('<end>')) {
                    ...
                    audioPlayer.value.markStreamEnd();
                    if (
                        outputData.value[outputData.value.length - 1]?.type === 'BOT' &&
                        outputData.value[outputData.value.length - 1].audio === '' &&
                        allVoice.value.length > 0
                ...
                if (data.choices[0]?.audio) {
                    console.log('audio return time -------------------------------', formatTimestamp(Date.now()));
    ...
    // 控制播放队列执行
    const playAudio = () => {
        console.log('剩余播放列表:', audioPlayQueue.value, formatTimestamp(Date.now()));
        ...
        if (!isEnd.value && base64List.value.length >= 2) {
            ...
            audioDOM.src = blob;
            audioDOM.play();
            console.error('前期合并后播放开始时间: ', formatTimestamp(Date.now()));
            audioDOM.onended = () => {
                console.error('前期合并后播放结束时间: ', formatTimestamp(Date.now()));
        ...
        if (isEnd.value && base64List.value.length >= 2) {
            ...
            audioDOM.src = blob;
            audioDOM.play();
            console.error('合并后播放开始时间: ', formatTimestamp(Date.now()));
            audioDOM.onended = () => {
                console.error('合并后播放结束时间: ', formatTimestamp(Date.now()));
    ...
    const truePlay = voice => {
        return new Promise(resolve => {
            ...
            console.error('播放开始时间:', formatTimestamp(Date.now()));
            audioDOM
                .play()
                .then(() => {
            ...
            // });
            audioDOM.onerror = () => {
                console.error('播放失败时间', formatTimestamp(Date.now()));
                resolve();
            };
            audioDOM.onended = () => {
                console.error('播放结束时间: ', formatTimestamp(Date.now()));
                // URL.revokeObjectURL(url);
                resolve();
            };
        });
    ...
    const dealImage = () => {
        ...
        if (videoImage.value.length > 5) {
            videoImage.value = videoImage.value.slice(5);
        }
