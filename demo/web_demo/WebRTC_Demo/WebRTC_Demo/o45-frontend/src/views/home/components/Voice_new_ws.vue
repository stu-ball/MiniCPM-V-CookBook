...
<script setup>
        ...
        fetchEventSource(url, {
            ...
            async onmessage(msg) {
                ...
                if (data.choices[0]?.text.includes('assistant')) {
                    console.warn('首token返回时间: ', formatTimestamp(Date.now()));
                }
                if (data.response_id) {
    ...
    const playAudio = () => {
        ...
        if (!isEnd.value && base64List.value.length >= 2) {
            console.log('合并情况1: ', base64List.value.length, formatTimestamp(Date.now()));
        ...
        if (isEnd.value && base64List.value.length >= 2) {
            console.log('合并情况2: ', base64List.value.length, formatTimestamp(Date.now()));
            const blob = mergeBase64ToBlob(base64List.value);
            // let audio = new Audio();
            audioDOM.src = blob;
            audioDOM.play();
            console.error('最后合并后播放开始时间: ', formatTimestamp(Date.now()));
            audioDOM.onended = () => {
        ...
        console.log('情况3: ', base64List.value.length);
