...
// Use synchronized time to format timestamp
...
// Algorithm requirement: Minimum video stream size is 448x448
const MIN_VIDEO_DIMENSION = 448;

// Video initialization config (prevents black screen)
const VIDEO_INIT_CONFIG = {
    // Video ready check timeout (ms)
    readyTimeout: {
        default: 5000, // Standard devices: 5s (longer wait)
        iPad: 8000 // iPad: 8s (slow camera init)
    },
    // Post-publish delay (ms) - skip initial black frames
    postPublishDelay: {
        default: 1500, // Standard devices: 1.5s (skip more initial black frames)
        iPad: 2000 // iPad: 2s
    },
    // Frame brightness threshold (0-255)
    brightnessThreshold: 15,
    // Black screen check interval (ms)
    blackScreenCheckInterval: 100,
    // Whether to send video ready signal to backend
    sendReadySignal: true,
    // Backend recommended wait time (ms) - for video_stream_ready signal
    backendStartDelay: {
        default: 2000, // Backend should wait extra 2s before sampling
        iPad: 3000 // iPad recommended wait 3s
    }
};

// Video resolution config (ensure algorithm requirements)
const VIDEO_RESOLUTION_CONFIG = {
    // High quality: 1280x720 (algorithm requirement, unified for all scenarios)
    high: VideoPresets.h720.resolution, // High quality: 1280x720 (algorithm requirement, unified for all scenarios)

    // Standard quality: 960x540 (backup config) - recommended for low-end devices
    standard: VideoPresets.h540.resolution, // Standard quality: 960x540 (recommended for low-end devices)

    // Minimum quality: 640x480 (4:3 ratio, meets minimum requirement)
    minimum: { width: 640, height: 480 }, // Minimum quality: 640x480 (4:3 ratio, meets minimum requirement)

    // Square: 640x640 (if algorithm needs square input)
    square: { width: 640, height: 640 }, // Square: 640x640 (if algorithm needs square input)

    // 🔧 New: Low performance mode (optimized for low-end Windows devices)
    lowPerformance: { width: 640, height: 480 } // Low performance mode: 480p, 15fps, optimized for low-end Windows devices
};

// 🔧 New: Performance monitoring data
let performanceMonitor = {
    frameDropCount: 0, // Dropped frame count
    lastCheckTime: 0, // Last check time
    degradeCount: 0, // Downgrade count
    currentLevel: null, // Current performance level
    staticLevel: null, // Static hardware level
    isDegraded: false // Is downgraded
};

/**
 * Get video resolution config
 * Returns resolution based on user's HD mode setting
 * @returns {Object} Resolution object {width, height}
 */
function getVideoResolution() {
    // Get HD mode config from localStorage
    const hdMode = localStorage.getItem('hdMode') === 'true';

    if (hdMode) {
        // HD mode: use 720p high quality capture
        console.log('📹 Video capture mode: HD (720p)');
        return VIDEO_RESOLUTION_CONFIG.high; // 1280x720
    } else {
        // Standard mode: use 540p standard quality capture
        console.log('📹 Video capture mode: Standard (540p)');
        return VIDEO_RESOLUTION_CONFIG.standard; // 960x540
    }
}

/**
 * Detect device performance level (static hardware check)
 * @returns {'high' | 'standard' | 'low'} Performance level
 */
function detectDevicePerformance() {
    // 🔧 Prefer checking manual setting
    const manualLevel = localStorage.getItem('forcePerformanceLevel');
    if (manualLevel && ['low', 'standard', 'high'].includes(manualLevel)) {
        console.log('🎯 Using manually set performance level:', manualLevel);
    ...
    if (isWindows && (cores <= 2 || memory <= 4)) {
        console.log('🔍 Detected low-end Windows device:', { cores, memory: memory + 'GB' });
        level = 'low';
    } else if (cores <= 4 || memory <= 8) {
        console.log('🔍 Detected medium-end device:', { cores, memory: memory + 'GB' });
        level = 'standard';
    } else {
        console.log('🔍 Detected high-end device:', { cores, memory: memory + 'GB' });
        level = 'high';
    }
...
/**
 * Dynamic performance monitoring: adjust performance level based on runtime
 * Checks frame rate, dropped frames, auto downgrade if performance is poor
 */
function monitorRuntimePerformance(videoTrack) {
    ...
    try {
        ...
        // If actual frame rate is much lower than expected (<70%), consider performance poor
        const performanceRatio = actualFrameRate / expectedFrameRate;

        console.log('📊 Performance monitoring:', {
            currentLevel: performanceMonitor.currentLevel,
            staticLevel: performanceMonitor.staticLevel,
            expectedFrameRate: expectedFrameRate,
            actualFrameRate: actualFrameRate.toFixed(1),
            performanceRatio: (performanceRatio * 100).toFixed(0) + '%',
            isDegraded: performanceMonitor.isDegraded
        });

        // Poor performance, consider downgrade
        if (performanceRatio < 0.7 && !performanceMonitor.isDegraded) {
            performanceMonitor.frameDropCount++;

            // After 3 consecutive detections of poor performance, trigger downgrade
            if (performanceMonitor.frameDropCount >= 3) {
                console.warn('⚠️ Performance is poor, downgrade recommended');
                suggestPerformanceDowngrade();
            }
        } else if (performanceRatio >= 0.9) {
            // Performance normal, reset counter
...
/**
 * Suggest performance downgrade (user notification)
 * ⚠️ Disabled: prevents black screen during video capture
 */
function suggestPerformanceDowngrade() {
    // 🔥 Disable performance downgrade: downgrading after capture starts may cause video stream interruption and black frames
    console.log('⚠️ Performance downgrade disabled (prevents black screen during video)');
    return;
    ...
    if (suggestedLevel !== currentLevel) {
        console.warn('💡 Performance suggestion: Current device performance is poor, recommend downgrade from', currentLevel, 'to', suggestedLevel);

        // Notify user (optional: auto downgrade or ask user)
        ...
        if (autoDowngrade) {
            ...
            // Trigger ElMessage notification (should be called in Vue component)
            ...
            // Suggest user manual adjustment
            console.log('💡 Suggestion: Run the following command in console to manually downgrade:');
...
/**
 * 🔧 Get current effective performance level (consider dynamic downgrade)
 */
 ...
 * - error: error from joinRoom
 * - localTracks: Array<LocalTrack>, contains audio and (optional) video tracks
 * - remoteTracks: { [participantSid]: Track[] }, contains all remote tracks
 * - localAudioActive: boolean, is local speaking
 * - remoteAudioActive: { [participantSid]: boolean }, who is speaking remotely
 * - videoFacing: 'user' | 'environment'
 * - audioEnabled: is local mic enabled
 * - videoEnabled: is local camera enabled
 * - messages: Array<{ from, payload }>, DataChannel text messages
 */
// Determine default camera facing based on device type
// - Mobile (phone/iPad): default to rear camera (environment)
// - PC/Desktop: default to front camera (user)
const getDefaultFacingMode = () => {
    ...
    if (isMobileDevice || isIPadDevice) {
        console.log('📱 Detected mobile device, defaulting to rear camera');
        return 'environment';
    } else {
        console.log('💻 Detected PC device, defaulting to front camera');
        return 'user';
    }
};
...
const state = reactive({
    ...
    localTracks: [],
    remoteTracks: {},
    // New: Is local speaking
    localAudioActive: false, // Is local speaking
    // New: Each remote participant's speaking state
    remoteAudioActive: {}, // Each remote participant's speaking state
    videoFacing: getDefaultFacingMode(), // Automatically select camera facing based on device type
    audioEnabled: true,
    videoEnabled: true,
    messages: [],
    chatMessages: [],
    messageIndex: -1, // Used to mark message index
    status: '',
    generateEnd: false, // Used to mark generation end state
    firstInit: true, // Marks if first initialization signal received
    modelInitialized: false, // Marks if model initialized successfully
    initConfig: null, // Stores initialization config
    muteRemoteAudio: false, // Marks if remote audio muted (effective after interruption)
    mode: 'audio', // Current call mode: 'audio' or 'video'
    // New: Record key audio timestamps by round
    ...
    // }
    audioRounds: [], // Per-round audio key timestamps
    pendingRoundIndex: -1, // Pending round index
    playEndSent: false, // Marks if play_end sent (prevents interference from delayed audio packets)
    playEndTimestamp: 0, // Records play_end sent timestamp
    currentRoundHasAudio: false, // Marks if current round has audio (for empty round detection)
    generateEndTimestamp: 0 // Records generate_end received time (for silence check protection)
});

let timer = null;
let noRobotTimer = null; // Timer for detecting if a bot has joined

// 🔧 New: Video health monitoring timer
let videoHealthCheckTimer = null;
let videoRecoveryAttempts = 0; // Video recovery attempt count
const MAX_VIDEO_RECOVERY_ATTEMPTS = 3; // Max recovery attempts
const VIDEO_HEALTH_CHECK_INTERVAL = 5000; // Check video health every 5 seconds

// 🔧 New: Memory management config
const MAX_AUDIO_ROUNDS = 20; // Keep up to 20 audio rounds
const MAX_CHAT_MESSAGES = 50; // Keep up to 50 chat messages
const MEMORY_CHECK_INTERVAL = 30000; // Check memory every 30 seconds
const MEMORY_THRESHOLD_MB = 200; // Trigger cleanup if memory exceeds 200MB
let memoryCheckTimer = null;

// Silence timeout config (dynamically adjusted by mode)
const SILENCE_CONFIG = {
    audio: {
        timeout: 800, // 🔧 Audio mode: 500ms → 800ms (tolerate multiple audio segment gaps and network delay)
        safetyDelay: 300, // 🔧 Safety delay: 200ms → 300ms (increase protection time)
        generateEndBuffer: 1000, // 🔧 generate_end buffer: 600ms → 1000ms (key: actual tests show delay up to 782ms)
        minAudioDuration: 600 // 🔧 New: Minimum protection time for short audio 600ms (two-character audio about 400-500ms)
    },
    video: {
        timeout: 1500, // 🔧 Video mode: 1200ms → 1500ms (video mode has higher network overhead)
        safetyDelay: 500, // 🔧 Safety delay: 400ms → 500ms (increase protection time)
        generateEndBuffer: 1200, // 🔧 generate_end buffer: 800ms → 1200ms (video mode has greater network delay)
        minAudioDuration: 800 // 🔧 New: Minimum protection time for short audio 800ms
    }
};
...
// 🔥 New: Audio end confirmation counter (prevent misjudgment)
...
// New: Register track subscription callback for immediate attach
export function registerTrackSubscribed(fn) {
    onTrackSubscribed = fn;
}

// Register local video element for forced refresh when switching camera
export function registerLocalVideoElement(el) {
    localVideoElement = el || null;
}

// Trigger registered cleanup callback (do not mistakenly trigger/overwrite with registerCleanup())
export function triggerCleanup(sids = []) {
    ...
    // 🔧 Cleanup video health monitoring timer
...
function startVideoHealthMonitoring() {
    if (videoHealthCheckTimer) {
        console.log('⚠️ Video health monitoring already running');
        return;
    }

    console.log('🏥 Start video health monitoring, check every 5 seconds');
...
function stopVideoHealthMonitoring() {
    if (videoHealthCheckTimer) {
        ...
        videoRecoveryAttempts = 0;
        console.log('🏥 Video health monitoring stopped');
    }
}
...
async function checkVideoHealth() {
    try {
        // Only check in video mode and when connected
        ...
        if (!videoTrack || !videoTrack.mediaStreamTrack) {
            console.warn('⚠️ Video track or MediaStreamTrack not found');
            return;
        }
        ...
        // 🔧 Disabled: dynamic performance monitoring (prevent black screen in the middle)
        // Performance downgrade may cause video stream interruption and black frames
        // monitorRuntimePerformance(videoTrack);

        // Check video track status
        if (readyState === 'ended' || !enabled) {
            console.error('❌ Video track abnormal:', { readyState, enabled });

            // Try auto recovery
            if (videoRecoveryAttempts < MAX_VIDEO_RECOVERY_ATTEMPTS) {
                videoRecoveryAttempts++;
                console.log(`🔄 Attempting auto recovery of video track (${videoRecoveryAttempts}/${MAX_VIDEO_RECOVERY_ATTEMPTS})`);
                await recoverVideoTrack();
            } else {
                console.error('❌ Video recovery failed, max attempts reached');
                stopVideoHealthMonitoring();
            }
        } else {
            // Video normal, reset recovery counter
            if (videoRecoveryAttempts > 0) {
                console.log('✅ Video track recovered, reset recovery counter');
                videoRecoveryAttempts = 0;
            }
        }

        // Check if video element is rendering normally
        if (localVideoElement) {
            const videoWidth = localVideoElement.videoWidth;
            const videoHeight = localVideoElement.videoHeight;

            if (videoWidth === 0 || videoHeight === 0) {
                console.warn('⚠️ Video element not rendering content:', { videoWidth, videoHeight });
            }
        }
    } catch (error) {
...
async function recoverVideoTrack() {
    try {
        console.log('🔄 Start recovering video track...');

        const currentVideoTrack = state.localTracks.find(t => t.kind === 'video');
        if (!currentVideoTrack) {
            console.error('❌ Current video track not found');
            return;
        }
        ...
        // Try using restartTrack (if supported)
        if (typeof currentVideoTrack.restartTrack === 'function') {
            console.log('🔄 Using restartTrack to recover video track');
            ...
            console.log('✅ Video track recovery successful (restartTrack)');
        } else {
            // Downgrade plan: recreate track
            console.log('🔄 Downgrade: use recreate track plan');
            ...
            // 🔥🔥🔥 New plan: decide whether to publish immediately based on model state
            // - 如果模型已初始化：立即发布新轨道
            // - 如果模型未初始化：只创建不发布，等待 model_init_success 信号
            if (state.modelInitialized) {
                // Model initialized, publish new track immediately
                ...
                // 🔥 Key fix: ensure video element is ready before publishing video
                if (localVideoElement) {
                    const currentReadyState = localVideoElement.readyState;
                    console.log(`📹 [Recover video - pre-publish check] video element readyState: ${currentReadyState}`);

                    if (currentReadyState < 2) {
                        console.warn(`⚠️ Video element not ready (readyState=${currentReadyState}), waiting for load...`);
                        ...
                        await new Promise(resolve => {
                            ...
                            const checkReady = () => {
                                if (localVideoElement.readyState >= 2) {
                                    console.log(
                                        `✅ Video element ready (readyState=${localVideoElement.readyState}), continue publishing`
                                    );
                                    resolve();
                                } else if (Date.now() - startTime > 3000) {
                                    console.warn(
                                        `⚠️ Video element wait timeout (readyState=${localVideoElement.readyState}), force continue publishing`
                                    );
                                    resolve();
                                } else {
                        ...
                        console.log(`✅ Video element ready (readyState=${currentReadyState}), can publish`);
                    }
                }
                ...
                console.log(
                    `%c✅ [Switch camera - new plan] Model initialized, new track published and sent`,
                    'color: #00ff00; font-weight: bold; font-size: 13px; background: #003300; padding: 2px 6px;'
                );
            } else {
                // Model not initialized, only create not publish
                console.log(
                    `%c⏳ [Switch camera - new plan] Model not initialized, new track created but not published (waiting for model_init_success)`,
            ...
            // Rebind to video element
            if (localVideoElement) {
                newTrack.attach(localVideoElement);
            }

            console.log('✅ Video track recovery successful (recreated)');
        }
    } catch (error) {
...
export function useLiveKit() {
    ...
    async function startScreenShare() {
        ...
        // Check LiveKit Room participation
        if (!state.room || !state.connected) {
            ElMessage({
                type: 'error',
                message: 'You must join the LiveKit Room before sharing your screen.',
                duration: 3000
            });
        ...
        try {
            ...
            // Attach screen share to local preview
            let preview = document.getElementById('screen-share-preview');
            if (!preview) {
                preview = document.createElement('video');
                preview.id = 'screen-share-preview';
                preview.autoplay = true;
                preview.muted = true;
                preview.style = 'position:fixed;bottom:16px;right:16px;width:320px;height:180px;z-index:9999;border:2px solid #647fff;background:#fff;';
            ...
            ElMessage({
                type: 'error',
                message: 'Screen sharing failed: ' + (e?.message || e),
                duration: 3000,
                showClose: true
            });
        }
            ...
            videoDevices.forEach(device => {
                ...
                // 📷 后置摄像头识别（iOS + Android，排除长焦）
                    ...
                    // iOS 识别模式
                        ...
                        cameraType = 'iOS 主摄广角';
                    } else if (label.includes('dual') || label.includes('triple') || label.includes('triple camera')) {
                        priority = 9; // 双广角/三镜头
                        cameraType = 'iOS 多镜头';
                    } else if (label.includes('ultra') || label.includes('ultra wide')) {
                        priority = 5; // 超广角（较低优先级，视野过宽）
                        cameraType = 'iOS 超广角';
                    }
                    // Android 其他识别模式
                    else if (label.includes('main') || label.includes('primary') || label.includes('main camera')) {
                        ...
                        priority = 7; // 其他后置摄像头
                        cameraType = 'Generic rear camera';
                    }
